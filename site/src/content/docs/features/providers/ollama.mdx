...
Ollama dispatch runs through the Claude CLI: the spawned `claude` subprocess is pointed at your Ollama endpoint via `ANTHROPIC_BASE_URL`, since Ollama speaks Claude's wire format directly. There is no in-process translation layer.
