...
| `/compact` | Compact the conversation. Works on every harness. On Claude Code the CLI handles it natively; on Codex Native Claudette issues the `thread/compact/start` JSON-RPC. The timeline renders a compaction divider when the compaction completes. A failed compaction surfaces a notice instead of a divider. |
